Describe 'Get-PercentageHeartBadge tests' {
    BeforeAll {
    ...
    # We don't need to test the specific emote it returns, in case I change it
    It 'Should return a non-empty string for any valid input (<name>)' -ForEach $testData {
        $result = Get-PercentageHeartBadge -Value $value -Max $max
